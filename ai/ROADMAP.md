...
| 2 | Backend agent loop | ~60–90 min | Bounded LLM+tool loop (iteration cap, timeout); every tool call recorded to a trace object (name, input, reasoning, result summary, timestamp) — this trace **is** the transparency requirement, not an afterthought bolted on later; API endpoint(s) for the frontend; loads `prompts/system_prompt.txt` directly as the system prompt; **grounding mechanical backstop** (added P27, `specs/agent-spec.md` §17) — after the final answer, regex-extract ID-shaped tokens and flag any not present in the trace, setting `grounding_check` on the task-level trace object |
